...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ background: s.bg, color: s.color, border: `1px solid ${s.border}`, borderRadius: 6, padding: "2px 10px", fontSize: 11, fontWeight: 700, letterSpacing: 1, fontFamily: "monospace" }}>{status}</span>
  );
}
...
export default function TradingJournal() {
  ...
  const stats = {
    total: trades.length, pending: trades.filter(t => t.status === "Pending").length,
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#e2e8f0", fontFamily: "'DM Mono', 'Fira Code', monospace", paddingBottom: 60 }}>

      {/* Header */}
      <div style={{ background: "linear-gradient(135deg, #0f0f1a 0%, #12121f 100%)", borderBottom: "1px solid #1e1e35", padding: "24px 20px 18px", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 11, color: "#6366f1", letterSpacing: 3, fontWeight: 700, marginBottom: 2 }}>◈ TRADING JOURNAL</div>
            <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: -1, color: "#f1f5f9" }}>Catatan Harian</div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={() => { setShowForm(v => !v); setFormError(""); }} style={{ background: showForm ? "#1e1e35" : "linear-gradient(135deg, #6366f1, #8b5cf6)", border: "none", borderRadius: 12, padding: "10px 16px", color: showForm ? "#94a3b8" : "#fff", fontFamily: "inherit", fontWeight: 700, fontSize: 12, cursor: "pointer", letterSpacing: 1, boxShadow: showForm ? "none" : "0 4px 20px rgba(99,102,241,0.4)" }}>
              {showForm ? "✕ Tutup" : "+ Tambah Order"}
            </button>
          </div>
        </div>
        {dbError && (
          <div style={{ marginTop: 10, padding: "8px 14px", background: "rgba(248,113,113,0.1)", border: "1px solid #7f1d1d", borderRadius: 8, fontSize: 12, color: "#f87171" }}>⚠ {dbError}</div>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, padding: "16px 16px 0" }}>
        {[{ label: "Total", value: stats.total, color: "#a78bfa" }, { label: "Pending", value: stats.pending, color: "#fbbf24" }, { label: "TP Hit", value: stats.tp, color: "#34d399" }, { label: "SL Hit", value: stats.sl, color: "#f87171" }].map(s => (
          <div key={s.label} style={{ background: "#12121f", border: "1px solid #1e1e35", borderRadius: 12, padding: "12px 10px", textAlign: "center" }}>
      ...
      <div style={{ padding: "10px 16px 0", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <div style={{ background: "#0f0f1a", border: "1px solid #1e1e35", borderRadius: 14, padding: "14px 16px" }}>
          ...
          {stats.winRate !== null ? (
            <>
              <div style={{ fontSize: 28, fontWeight: 900, lineHeight: 1, marginBottom: 6, color: parseFloat(stats.winRate) >= 50 ? "#34d399" : "#f87171" }}>{stats.winRate}%</div>
              <div style={{ fontSize: 10, color: "#475569" }}>{stats.tp}W / {stats.sl}L dari {stats.closedCount} closed</div>
        ...
        <div style={{ background: "#0f0f1a", border: "1px solid #1e1e35", borderRadius: 14, padding: "14px 16px" }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 900, lineHeight: 1, marginBottom: 6, color: pnlColor(stats.totalPnl) }}>
            {stats.totalPnl >= 0 ? "+" : ""}{stats.totalPnl.toFixed(2)}<span style={{ fontSize: 11, color: "#475569", fontWeight: 400, marginLeft: 4 }}>USDT</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10 }}><span style={{ color: "#475569" }}>Profit</span><span style={{ color: "#34d399", fontWeight: 700 }}>+{stats.totalProfit.toFixed(2)} USDT</span></div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10 }}><span style={{ color: "#475569" }}>Loss</span><span style={{ color: "#f87171", fontWeight: 700 }}>{stats.totalLoss.toFixed(2)} USDT</span></div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, padding: "16px 16px 8px", overflowX: "auto" }}>
        {["All", "Pending", "Filled", "TP Hit", "SL Hit", "Cancelled"].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{ background: filter === f ? "#6366f1" : "#12121f", border: `1px solid ${filter === f ? "#6366f1" : "#1e1e35"}`, borderRadius: 20, padding: "5px 14px", color: filter === f ? "#fff" : "#64748b", fontFamily: "inherit", fontSize: 11, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", letterSpacing: 0.5 }}>{f}</button>
        ))}
      </div>
      ...
      {!loading && (
        <div style={{ padding: "0 16px", display: "flex", flexDirection: "column", gap: 10 }}>
          {filteredTrades.length === 0 && <div style={{ textAlign: "center", color: "#334155", padding: "40px 0", fontSize: 13 }}>Belum ada trade. Klik "+ Tambah Order" untuk mulai.</div>}
